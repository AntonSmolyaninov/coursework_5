...
class HabitModelTest(TestCase):
    ...
    def test_create_valid_habit(self):
        habit = Habit.objects.create(
            user=self.user, place="Home", time="08:00:00", action="Exercise", execution_time=60, reward="Reward"
        )
    ...
    def test_execution_time_exceeds_limit(self):
        with self.assertRaises(ValidationError):
            Habit.objects.create(
                user=self.user, place="Home", time="08:00:00", action="Exercise", execution_time=150, reward="Reward"
            )
...
class HabitAPITest(APITestCase):
    ...
    def test_my_habits_endpoint(self):
        Habit.objects.create(
            user=self.user, place="Home", time="08:00:00", action="My habit", execution_time=60, reward="Reward"
        )
